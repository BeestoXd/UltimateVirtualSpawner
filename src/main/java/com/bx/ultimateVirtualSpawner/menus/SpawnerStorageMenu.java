...
public class SpawnerStorageMenu extends BaseMenu {
    ...
    public SpawnerStorageMenu(UltimateVirtualSpawner plugin, long spawnerId, int page) {
    ...
    public static ItemStack applyStorageMeta(UltimateVirtualSpawner plugin, SpawnerInstance instance, Material material, int amount) {
    ...
    public void handleInventoryClick(InventoryClickEvent event) {
        ...
        if (rawSlot >= lastRow || rawSlot == backSlot || rawSlot == filterSlot || rawSlot == collectSlot || rawSlot == prevSlot || rawSlot == nextSlot || rawSlot == dropSlot || rawSlot == sellSlot) {
            ...
            if (rawSlot == backSlot) {
                ...
                player.sendMessage(plugin.getSpawnerManager().collectAllLoot(player, instance).message());
                ...
                player.sendMessage(plugin.getSpawnerManager().dropAllLoot(player, instance).message());
                new SpawnerStorageMenu(plugin, spawnerId, page).open(player);
        ...
        if (rawSlot >= 0 && rawSlot < lastRow) {
            ...
            if (clickType == ClickType.SHIFT_RIGHT || clickType == ClickType.MIDDLE) {
                if (slotItem != null && !slotItem.getType().isAir()) {
                    ...
                    plugin.getMessageManager().send(player, "SPAWNER.FILTER_TOGGLED",
                            "material", plugin.getWorthManager().prettifyMaterial(slotItem.getType()),
                            "status", plugin.getMessageManager().raw(!currentState
                                    ? "SPAWNER.FILTER_STATUS_DISABLED"
                                    : "SPAWNER.FILTER_STATUS_ENABLED"));
