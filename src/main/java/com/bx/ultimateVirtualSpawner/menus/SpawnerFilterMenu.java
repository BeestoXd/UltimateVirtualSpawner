...
public class SpawnerFilterMenu extends BaseMenu {
    ...
    public SpawnerFilterMenu(UltimateVirtualSpawner plugin, long spawnerId, int returnPage) {
    ...
    public void handleClick(int slot, Player player, ClickType clickType) {
        ...
        if (slot == enableAllSlot) {
            if (def != null) {
                ...
                plugin.getMessageManager().send(player, "SPAWNER.FILTER_ENABLED_ALL");
            }
        ...
        if (slot == disableAllSlot) {
            if (def != null) {
                ...
                instance.setUpdatedAt(System.currentTimeMillis());
                plugin.getSpawnerManager().saveSpawnerAndLoot(instance);
                plugin.getMessageManager().send(player, "SPAWNER.FILTER_DISABLED_ALL");
            }
            plugin.getSpawnerManager().playFilterToggleSound(player);
            new SpawnerFilterMenu(plugin, spawnerId, returnPage).open(player);
            return;
        ...
        if (dropSlotMap.containsKey(slot)) {
            ...
            plugin.getMessageManager().send(player, "SPAWNER.FILTER_TOGGLED",
                    "material", plugin.getWorthManager().prettifyMaterial(drop.material()),
                    "status", plugin.getMessageManager().raw(newState
